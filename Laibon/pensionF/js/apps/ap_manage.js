function AP_MANAGE(){
	var apiurl="http://192.168.1.224:8080/nursecare/wifi/advancedSearchAndSplit.do",
	    totalPage,//总共页数
	    curPage,//当前页数
	    perPapernum,//每页多少条数据
    ...
    /*getApData（pram1，pram2，pram3，pram4，pram5，pram6，）
              功能描述：根据不同条件搜索显示数据
      ...
      pram8：位置描述
     * */
	function getApData(type,currentpage,perpage,ip,mac,ssid,building_Id,floor_Id,location_Desc){
		//cp=#&ps=#&ip=#&mac=#&ssid=#&locationDesc=#&buildingId=#&floorId=#
		...
		(mac ==undefined) ? ap_mac=" " : (ap_mac="&mac="+mac);
		(ssid ==undefined) ? ap_ssid=" " : (ap_ssid="&ssid="+ssid);
		...
		if(type==1){//简单搜索
			go_searchpageurl='http://192.168.1.224:8080/nursecare/wifi/simpleSearchAndSplit.do?searchmsg=1,10,'+$('#tags').val();
		}else{
			go_searchpageurl=apiurl+'?'+currentpage_param+perpage_param+ip_param+ap_mac+ap_ssid+buildingId_param+floorId_param+locationDesc_param+'';
		}
	...
	function parseApdata(apdata){
		...
		for (var i=0;i<dataobj.length;i++) {
			//if(dataobj[i].locationType=="0"+''){//室内
				...
				+dataobj[i].ip+"</td><td>"+dataobj[i].mac+"</td><td>"+dataobj[i].ssid+"</td><td>"
				+dataobj[i].building+"</td><td>"+dataobj[i].floor+"</td><td>"
				+dataobj[i].locationDesc+"</td><td><img src='../../img/delete.png' id="+dataobj[i].id+" style='max-width: 20px;'/>"
	   ...
	   /*编辑*/
        $("td img").click(function(){
        	...
        	if(($(this).prop('src').indexOf("edit.png")) > 0 ){
	        	...
	        	if($(this).prop('id')){
	        		...
	        		for (var j=0;j<dataobj.length;j++) {
	        			...
	        			if($(this).prop('id')==dataobj[j].id){
	        				...
	        				$("#editMAC").val(dataobj[j].mac);
	        				$("#editSSID").val(dataobj[j].ssid);
        		...
        		if($(this).prop('id')) {
        			$.ajax({
        				url: 'http://192.168.1.224:8080/nursecare/wifi/deleteMany.do?wifiIds=' + $(this).prop('id'),
        				type: 'GET',
        				//xml, html, script, json, jsonp, text
        				dataType: 'text',
        				timeout: 3000,
	...
	this.init=function(){		
		getApData();//初始化不带参数表格默认显示第一页
		$("#search-Go").click(function(){
        	getApData(0,$("#tag").val(),$("#row_num").find("option:selected").text(),$("#tags").val() );//带参数搜索精确页的数据
        ...
        $("#row_num").change(function(){  //为Select添加事件，当选择其中一项 时触发
	                ...
	                getApData(0,'1',checkText);
				});
	    /*简单搜索*/	   
	    $("#searchIcon").click(function(){
	    	getApData(1,$("#row_num").find("option:selected").text(),$("#tags").val());
	    });
	    ...
	    $("#hightSearch").click(function(){
	    	console.log($("#input_ip").val()+"--"+$(".builds").find("option:selected").val()+"--"+$(".floors").find("option:selected").val()+"--"+$("#ipclocation").val());
	    	getApData(0,'1','10',$("#input_ip").val(),$("#input_mac").val(),$("#input_ssid").val(),$(".builds").find("option:selected").val(),$(".floors").find("option:selected").val(),$("#input_location").val());
	    });
	    /*高级搜索下的返回*/
	   ...
	   $("#reset").click(function(){
		...
		$("#addHold").click(function(){
			...
			if(positionType){
				if(positionType=='1'+''){//室外---ip,building_id,floor_id,安装位置
					//拼接的AP信息,结构应该为：ip,mac,ssid,building_id,floor_id,location_desc; （各字段不能为空）
					if($("#addIp").val()&&$("#addMac").val()&&$("#addSsid").val()&&$("#addLocation").val()){
						holdINfo=$("#addIp").val()+","+$("#addMac").val()+','+$("#addSsid").val()+','+$("#addLocation").val();
					}else{
						alert("各输入不能为空！")
					}
				}else{//室内---ip,安装位置 
					holdINfo=$("#addIp").val()+","+$("#addMac").val()+','+$("#addSsid").val()+','+$(".addBuild").find("option:selected").val()+","+$(".addFloor").find("option:selected").val()+","+$("#addLocation").val();
				    
				}
				console.log("保存的信息："+holdINfo);
			}
			$.ajax({
				url: 'http://192.168.1.224:8080/nursecare/wifi/insertNewWifi.do?newWifi='+holdINfo,
				type: 'GET',
				//xml, html, script, json, jsonp, text
				dataType: 'text',
				timeout: 3000,
	    ...
	    $("#deleteBtn").click(function(){
			...
			if(ids){
				$.ajax({
					url: 'http://192.168.1.224:8080/nursecare/wifi/deleteMany.do?wifiIds=' + ids,
					type: 'GET',
					//xml, html, script, json, jsonp, text
					dataType: 'text',
					timeout: 3000,
	   ...
	   $("td img").click(function(){
	    ...
	    $("#editSave").click(function(){
	    	var edit_holdINfo;
	    	var positionType=0;          	    		 
	    	edit_holdINfo = AP_id+ ","+$("#editIP").val() + "," + $("#editMAC").val() + "," +$("#editSSID").val() + "," +$("#editBuild").find("option:selected").val() + "," + $(".editFloor").find("option:selected").val() + "," + $("#editLocation").val();	    		   		
	    	var updateUrl='http://192.168.1.224:8080/nursecare/wifi/updateWifi.do?wifi=' + edit_holdINfo;
	    	//alert("编辑保存的信息：" + updateUrl);
	    	$.ajax({
	    		url: updateUrl,
	    		type: 'GET',
	    		//xml, html, script, json, jsonp, text
	    		...
	    		success: function(data, textStatus) {
	    			var data_ap=JSON.parse(data);
	    			if(data_ap.status=='103'+''){
	    				alert("AP信息修改失败！");
	    			}else{
	    				alert("AP信息修改成功！");
	    			}
