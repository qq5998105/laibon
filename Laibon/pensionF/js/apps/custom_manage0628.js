function CUSTOM_MANAGE(){
     ...
     * */
	function getCustomData(type,currentpage,perpage,patientNameKey,phoneNumberKey,IMEIKey,beginDate,endDate){
	...
	function parseCustomdata(customdata){
		...
		for (var i=0;i<dataobj.length;i++) {							
					tr_html="<tr><td><input name='subBox' type='checkbox' value="+dataobj[i].watchId+" />"+(i+1)+"</td><td>"
						...
						+dataobj[i].sosType+"</td><td><button class='stopBtn' id="+'stat_'+dataobj[i].watchId+">查看</button><img src='../../img/delete.png' id="+'delet_'+dataobj[i].watchId+" style='max-width: 20px;'/>"
            ...
            success: function(data, textStatus) {
			    ...
			    for(var i = 0; i < dataobj.length; i++) {			    	
			    	tr_html="<tr><td><input name='subBox' type='checkbox' value="+dataobj[i].watchId+" />"+(i+1)+"</td><td>"
						...
						+dataobj[i].sosNumber+"</td><td>"+dataobj[i].type+"</td><td>"
						+dataobj[i].IMEI+"</td><td>"						
						+dataobj[i].sosTypeId+"</td><td><button class='stopBtn' id="+'stat_'+dataobj[i].watchId+">查看</button><img src='../../img/delete.png' id="+'delet_'+dataobj[i].watchId+" style='max-width: 20px;'/>"
						+"<a href='#modal-container-998611' data-toggle='modal'><img src='../../img/edit.png' id="+dataobj[i].watchId+" style='max-width: 20px;margin-left: 10px;'/></a></td></tr>";				
			    	$(".gridList").append(tr_html);
			    	//}
			    }
	...
	this.init=function(){		
		getCustomData();//初始化不带参数表格默认显示第一页
        ...
        $("#row_num").change(function(){  //为Select添加事件，当选择其中一项 时触发
	                ...
	                //getCustomData	(0,'1',checkText);
	                getCustomData(0,'1',checkText,$("#inputName").val());
				});
	    /*简单搜索*/	   
	    ...
	    $("#hightSearch").click(function(){
	    	...
	    	getCustomData(0,'1',$("#row_num").find("option:selected").text(),$("#inputName").val(),$("#inputPhonenum").val(),$("#inputImei").val(),$("#_easyui_textbox_input1").val(),$("#_easyui_textbox_input2").val());
	    });
	    /*高级搜索下的返回*/
