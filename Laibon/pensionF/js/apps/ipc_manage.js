function IPC_MANAGE(){
	...
	function getIpcData(currentpage,perpage,ip,building_Id,floor_Id,location_Desc){
		...
		(typeof(currentpage)=='undefined') ? (currentpage_param="1") : (currentpage_param=("cp="+currentpage));
		(perpage ==undefined) ? perpage_param="10" : (perpage_param="&ps="+perpage);
        	...
        	if(($(this).prop('src').indexOf("edit.png")) > 0 ){
	        	...
	        	if($(this).prop('id')){
	        		...
	        		$.ajax({
	        			...
	        			success: function(data, textStatus) {
	        				console.log("ipc信息载入："+data);
	        				var IPcObj=JSON.parse(data).body;	        				
	        				//把对应的ipc信息放入编辑页面       				
	        				$("#editIP").val(IPcObj.ip);
	        				$("#editLocation").val(IPcObj.locationDesc);
	        				console.log(IPcObj.building);
	        				curBuild=IPcObj.building; 
	        				curFloor=IPcObj.floor;	        				
	        			},
	        			error: function(XMLHttpRequest, textStatus, errorThrown) {
	        				//timeout也会进入                        	           	
	...
	this.init=function(){		
	   ...
	   $("#reset").click(function(){
	    ...
	    $("#editSave").click(function(){
	    	var edit_holdINfo;
	    	var positionType=0;          	    		 
	    	edit_holdINfo = Ipc_id+ ","+$("#editIP").val() + "," + $("#editBuild").find("option:selected").val() + "," + $(".editFloor").find("option:selected").val() + "," + $("#editLocation").val();	    		   		
	    	IPcUrl='http://192.168.1.224:8080/nursecare/IPC/updateIPC.do?positionType=' + positionType + '&newIPC=' + edit_holdINfo;
	    	//alert("编辑保存的信息：" + IPcUrl);
	    	$.ajax({
	    		url: IPcUrl,
	    		type: 'GET',
	    		//xml, html, script, json, jsonp, text
	    		...
	    		success: function(data, textStatus) {
	    			var ipc_obj=JSON.parse(data);
	    			if(ipc_obj.status=="000"){
	    				alert("IPC修改成功!");
	    			}else{
	    				alert("IPC修改失败!");
	    			}	    			
