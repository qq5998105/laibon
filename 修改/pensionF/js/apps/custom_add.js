...
//					console.log("zhang.获取楼层..失败....");
		...
		$.ajax({
			...
			success: function(res) {
				...
				for(var i = 0; i < parseInt(staffDatelist.length); i++) {
					$(".personGrid").append("<tr><td class='radioData'><input type='radio' name='staff' class='radioChange'></td><td class='radioNum'>" + staffDatelist[i].num + "</td><td class='radioName'>" + staffDatelist[i].name + "</td><td>" + staffDatelist[i].birthday + "</td></tr>");
				}
	...
	function save_Submit() {
		...
		var watchSettings = {
//			"sosName": $(".SOS-change-name").val();
//			"sosNumber":$(".SOS-change-number").val();
		};
		var patientHealth = {
		...
		$.ajax({
			url: window.localStorage["ip:port"] + '/lb/patient/insert.do',
			type: 'POST',
			data: infoobj,
			//xml, html, script, json, jsonp, text
			dataType: 'text',
